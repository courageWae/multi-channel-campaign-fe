...
const Table = ({
  ...
  const shareLink = (item) => {
    ...
    window.navigator.clipboard.writeText(
      `${window.location.origin}/preview/${item.id}`
    );
    ...
    if (status === 1) {
      ...
      return (
        ...
        >
          ...
          <span className="ml-2 font-semibold">Approved</span>
        </button>
      );
    } else {
      return (
      ...
      <ActionBtnWrapper>
        {openActionMenu === item.id && (
          <MenuOptions id="menuOptions">
            ...
            {item.status === 1 && (
              <MenuItem onClick={() => approvalModel(item)}>
                <FaCheckCircle className="mr-2 text-gray-700" /> Approve Survey
              </MenuItem>
            )}
            <Seperator />
  ...
  const getShortCode = (item) => {
    ...
    if (item.short_code_status === 1 && expired === false) {
      return (
        <button
          type="button"
    ...
    } else if (item.short_code_status === 1 && expired === true) {
      return (
        <button
          type="button"
  ...
  const getUssdStatus = (item) => {
    if (item.status === 1) {
      return (
        <LiveButton children={"Pending"} status={item.short_code_status} />
      );
