...
const UpdateStatus = ({
    ...
    const ActionDevice = () =>
        actionDeviceMutate({ id: selectedData?.id, });
        ...
        <Model width={"w-11/12 max-w-md"} setOpenModel={setOpenActionModel} title="Update Status">
            ...
            {!actionDeviceLoading && (
                <>
                    ...
                    <SubTitle>
                        You are about to {selectedData.status == "1" ? "Inactive" : "Active"} selected user.
                        {selectedData.status == "1"
                            ? "They will no longer be able to login to the system but will still be displayed in your admin users list."
                            : "They will now be able to login to the system."}
                    </SubTitle>
