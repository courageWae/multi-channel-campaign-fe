            ...
            <><div>
                ...
                <div className="text-sm text-gray-500">#{item.id} • <span className="font-medium">{status}</span> on {moment(item.updated_at).format('MMMM Do YYYY, h:mm:ss a')}</div><div className="flex space-x-2 mt-1">
                    {item.status == Config.CampaignStatus.Pending && item.is_scheduled == Config.CampaignStatus.isSchedule && user.type == Config.UserType.ClientUser && (
                        <><Link to={`/campaign/whatsapp/edit/${item.id}`} className="text-gray-600 hover:text-blue-600 underline">Edit</Link><span className="text-gray-400">•</span></>)}
                    ...
                    <Link to={`/campaign/whatsapp/report/${item.id}`} className="text-gray-600 hover:text-blue-600 underline">Report</Link>
                </div>
            </div>
            </>
        );
    ...
    const columns = React.useMemo(
        () => [
            ...
            },
            {
                ...
                accessor: user.type === Config.UserType.SuperAdminUser ? "userName" : "undefined",
            },
            {
                Header: "Message Sent",
                accessor: "recipients",
