...
const StatusUpdate = ({
    ...
    const newStatus = selectedData?.status == "1" ? "2" : "1";
    const ActionDevice = () =>

        actionDeviceMutate({ connectionId: selectedData?.id, status: newStatus });
        ...
        <Model width={"w-11/12 max-w-md"} setOpenModel={setOpenActionModel} title="Update Status">
            ...
            {!actionDeviceLoading && (
                <>
                    ...
                    <SubTitle>
                        You are about to {selectedData.status == "2" ? "Active" : "Inactive"} selected connection.
                        {selectedData.status == "1"
                            ? "They will no longer be able to receive sms/email campaigns but will still be displayed in your connection."
                            : "They will now be able to receive sms/email connection."}
                    </SubTitle>
