...
const ActivityTime = tw.span`text-gray-500 text-xs w-24`;
const ActivityContent = tw.div`flex items-center space-x-4`;
const ActivityIcon = tw.div`flex-shrink-0 text-lg`;
const ActivityDetails = tw.div``;
const ActivityTitle = tw.h4`text-sm font-semibold text-gray-700`;
const ActivityDescription = tw.p`text-xs text-gray-500`;

const socialMediaType = (value) => {
  if (value === "1") {
    return (
      <button className="flex items-center justify-center w-10 h-10 transition-all duration-300 bg-white rounded-lg shadow-md shadow-gray-200 group">
        <svg
    ...
    return (
      <button className="flex items-center justify-center w-10 h-10 transition-all duration-300 bg-white rounded-lg shadow-md group shadow-gray-200">
        <svg
          className="transition-all duration-300 group-hover:scale-110"
          width="28"
          height="28"
      ...
      {socialMedia.length === 0 ? (
        ...
        <>
          {socialMedia.map((item, index) => (
            <ActivityItem key={index}>
              <ActivityTime>{moment(item.created_at).fromNow()}</ActivityTime>
              <ActivityContent>
                <ActivityIcon>{socialMediaType(item.post_type)}</ActivityIcon>
                <ActivityDetails>
                  <ActivityTitle>{item.socialname}</ActivityTitle>
                  <ActivityDescription>{item.text.length > 100 ? item.text.slice(0, 100) + "..." : item.text}</ActivityDescription>
                </ActivityDetails>
              </ActivityContent>
            </ActivityItem>
          ))}
