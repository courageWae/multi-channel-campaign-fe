...
const Table = ({
      ...
      <ActionBtnWrapper>
        {openActionMenu === item.id && (
          <MenuOptions id="menuOptions">
            <Link to={`/email-template/edit/${item.id}`}>
              <MenuItem>
                <CiEdit className="mr-2 text-gray-700" /> Edit Template
              </MenuItem>
            </Link>
            <Seperator />
            <MenuItem onClick={() => DeleteThisEmailTemplate(item.id)}>
              <MdDelete className="mr-2 text-red-700" /> Delete Template
            </MenuItem>
          </MenuOptions>
        )}
  ...
  const getStatus = (status, reason) => {
    if (status === 1) {
      return (
        <PendingBadge>
          <FaRegCircle className="inline w-4 h-4 mr-1" />
          Not Verified
        </PendingBadge>
      );
    } else if (status === 2) {
      return (
        <SuccessBadge>
          <FaRegCheckCircle className="inline w-4 h-4 mr-1" />
          Verified
        </SuccessBadge>
      );
    } else {
      return (
        ...
        >
          <RxCrossCircled className="inline w-4 h-4 mr-1" />
          Not Verified
          <Tooltip
  ...
  const getTemplateName = (name, status, email) => {
    return (
      <>
        <div className="flex items-center">
          <div className="flex-shrink-0 w-10 h-10 mb-3 mr-2 sm:mr-3">
            <div
          ...
          <div className="flex flex-col font-bold text-gray-600 align-items-start">
  ...
  useEffect(() => {
    if (ApiData) {
      const tempData = ApiData.map((item) => ({
        ...
        fromName: getTemplateName(item.name, item.status, item.email_id),
