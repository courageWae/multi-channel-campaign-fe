...
import { MdDelete, MdArrowBack, MdArrowForward, MdDriveFileRenameOutline, MdOutlinePermMedia, MdRemoveRedEye } from "react-icons/md";
...
const Table = ({
            ...
            <ActionBtnWrapper>
                {openActionMenu === item.id && (
                    <MenuOptions id="menuOptions">
                        <Link to={`/whatsapp-view-template/${item.id}`}>
                            <MenuItem>
        ...
        if (status === 2) {
            return (
                <>
                    ...
                    {/* <InactiveStatus data-tooltip-id="smsInactive" data-tooltip-content={Config.TemplateStatusText.SMSInactive}>
                        <FaTimesCircle className="mr-1" />
                        SMS
                        <Tooltip id="smsInactive" place="top" effect="solid" />
                    </InactiveStatus> */}
                    <InactiveStatus className="mt-1" data-tooltip-id="whatsappInactive" data-tooltip-content={Config.TemplateStatusText.WhatsappInactive}  data-tooltip-delay-show={1000}>
                        <FaTimesCircle className="mr-1" />
                        WhatsApp user initiated
                        <Tooltip id="whatsappInactive" place="top" effect="solid" />
                    </InactiveStatus></>
            );
        } else if (status == 1) {
            return (
                <>
                    ...
                    {/* {contentType == 1 && (
                        <ActiveStatus data-tooltip-id="smsActive" data-tooltip-content={Config.TemplateStatusText.SMSActive}>
                            <FaCheckCircle className="mr-1" />
                            SMS
                            <Tooltip id="smsActive" place="top" effect="solid" />
                        </ActiveStatus>)} */}
                    <ActiveStatus className="mt-1" data-tooltip-id="whatsappActive" data-tooltip-content={Config.TemplateStatusText.WhatsappActive}  data-tooltip-delay-show={1000}>
                        <FaCheckCircle className="mr-1" />
                        WhatsApp user initiated
                        <Tooltip id="whatsappActive" place="top" effect="solid" />
                    </ActiveStatus></>
            );
        }
        else if (status == 3) {
            return (
                <>
                    ...
                    <PendingStatus className="mt-1" data-tooltip-id="whatsappPending" data-tooltip-content={Config.TemplateStatusText.WhatsappPending}  data-tooltip-delay-show={1000}>
                        <FaRegCircle className="mr-1" />
                        WhatsApp user initiated
                        <Tooltip id="whatsappPending" place="top" effect="solid" />
                    </PendingStatus></>
            );
        }
        else {
            return (
                <>
                    ...
                    <RejectStatus className="mt-1" data-tooltip-id="whatsappRejected" data-tooltip-content={Config.TemplateStatusText.WhatsappRejected}  data-tooltip-delay-show={1000}>
                        <FaTimesCircle className="mr-1" />
                        WhatsApp user initiated
                        <Tooltip id="whatsappRejected" place="top" effect="solid" />
                    </RejectStatus></>
            )
        }
    };
    ...
    const getType = (contentType) => {
        if (contentType === 1) {
            return <TextBadge><PiTextTBold className="mr-2" />Text</TextBadge>;
        } else {
            return <MediaBadge><MdOutlinePermMedia className="mr-2" />Media</MediaBadge>;
        }
    };
