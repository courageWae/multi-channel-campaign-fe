...
import Table from "components/AdminSurveyManagement/Table";
import ApproveSurveyModal from "components/AdminSurveyManagement/ApproveSurveyModal";
...
const SurveyList = () => {
  ...
  // ------ USSD LIVE MODE -------
  const approvalFunction = async (values) =>
    await axios.post(
      `${Config.apiUrl}/ussd/action`,
      values,
      {
        headers: {
          "Content-Type": "application/json",
  ...
  const approvalError = (data) => {
    setOpenApprovalModel(false);
    setSelectedData("");
    toast.error(data?.data?.msg || "An Error Occurred");
  };

  const { isLoading: approvalLoading, mutate: SurveyApprovalMutate } = useMutation(
    approvalFunction,
    {
        ...
        <Page>
          ...
          <Content>
...

            {
              openApprovalModel && (
                <ApproveSurveyModal
                  setOpenApprovalModel={setOpenApprovalModel}
                  approvalLoading={approvalLoading}
                  selectedData={selectedData}
                  SurveyApprovalMutate={SurveyApprovalMutate}
                />
              )
            }
