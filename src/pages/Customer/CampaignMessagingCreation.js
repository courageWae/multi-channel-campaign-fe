...
const MessagingCampaign = () => {
  ...
  useEffect(() => {
    ...
    if (
      (permissions.includes(Config.Permissions.SMS) &&
  ...
  useEffect(() => {
    if (data) {
      ...
      const sender = data.data?.senderList?.find(
        (sender) => sender.id === initialValues.sender
      );
  ...
  const ScheduleFunction = async (values) => {
    return await axios.post(`${Config.apiUrl}/campaign/sms/create`, values, {
      headers: {
    ...
    <Container>
      <Page>
        ...
        {step === 1 && (
          <Contents>
            <ContentHeader>
              <HeaderTitle>Create a SMS campaign</HeaderTitle>
              <HeaderSubTitle className="mt-6">
                Reach out to your customers on SMS. Share important news,
