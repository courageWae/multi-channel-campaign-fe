...
import FbPostModel from "components/FbPost/FbPostModel";
import CreatePostModal from "components/FbPost/FbPostModel";
import InstaPostModal from "components/InstagramPost/InstagramPostModel";
import NoPermissionModal from "components/NoPermission";

const PostContainer = tw.div`bg-white h-[26rem]  border border-gray-200 rounded-lg shadow-lg lg:w-2/3 md:w-1/2 w-full`;
const Header = tw.div`flex items-center space-x-2 px-4 pt-4 mb-4`;
...
const DropzoneContainer = tw.div`flex w-full items-center h-32  p-4 border-2 border-dotted border-gray-300 hover:border-blue-500 rounded mb-4 text-center`;
...
const PreviewImage = tw.img`w-full h-32 object-cover rounded mb-4`;
const FbPreviewImage = tw.img`w-full h-28 object-cover`;
...
const CreateFbPost = () => {
  ...
  const mockUser = {
    facebookPage: "./images/facebook-icon.png",
    facebookGroup: "./images/facebook-icon.png",
  ...
  const Avatar = ({ user }) => {
    ...
    const handleFacebookSelection = (type) => {
      setSelectedFacebook(type);
    };
    ...
    return (
      ...
      >
        <AvatarContainer
          className={`avatar ${selectedFacebook === "page" ? "selected" : ""}`}
          onClick={() => handleFacebookSelection("page")}
        >
          <img src={user.facebookPage} alt="Facebook Page" />
          {/* <span className="facebook-icon"></span> */}
        </AvatarContainer>

        <AvatarContainer
          className={`avatar ${selectedFacebook === "group" ? "selected" : ""}`}
          onClick={() => handleFacebookSelection("group")}
        >
          <img src={user.facebookGroup} alt="Facebook Group" />
          {/* <span className="facebook-icon"></span> */}
        </AvatarContainer>
      </div>
    );
      ...
      return (
        <div className="flex gap-1">
          {images.map((image) => (
            <img
              key={image.name}
              className="object-cover w-1/2 h-28"
              src={image.preview}
              alt="Preview"
            />
          ))}
    ...
    if (imageCount === 3) {
      return (
        <div className="grid grid-cols-2 gap-1">
          <img
            className="object-cover w-full col-span-2 h-28"
            src={images[0].preview}
            alt="Preview"
          />
          <img
      ...
      return (
        <div className="grid grid-cols-3 gap-1">
          {images.slice(0, 1).map((image) => (
            <img
              key={image.name}
              className="object-cover w-full col-span-3 h-28"
              src={image.preview}
              alt="Preview"
            />
          ))}
        ...
        <div className="grid grid-cols-3 gap-1">
          ...
          <div className="relative">
            <img
              ...
              alt="Preview"
            />
            <div className="absolute inset-0 flex items-center justify-center text-2xl text-white bg-black bg-opacity-50">
              +{imageCount - 5}
            </div>
          </div>
        </div>
      ...
      <Page>
        ...
        <Content>
          ...
          <div className="App">
            ...
            {isInstaModalOpen && (
              <InstaPostModal onClose={() => setInstaModalOpen(false)} />
            )}
          </div>
          <ContentHeader>
            <div className="flex items-center justify-between">
              <HeaderTitle>Create Facebook Post</HeaderTitle>
            </div>
            <LearnMoreLink>
              <Link to="" className="flex items-center gap-1">
                <p className="underline hover:text-topBar-purple">
                  Why is it necessary to manually enter a message for Facebook?
                </p>
              ...
              {showAiAssistant ? (
                ...
                <div className="h-[26rem] overflow-y-auto">
                  ...
                  <div className="mx-4 mb-4 bg-white border border-gray-200 rounded-lg">
                      ...
                      <div className="flex items-center space-x-2">
                        ...
                        <div>
                          ...
                          <div className="flex text-sm text-gray-500">
                            Just Now · <IoEarth className="mt-1 ml-1" />
                          </div>
                        </div>
                      </div>
                    ...
                    <div className="flex items-center justify-between px-4 py-1 my-2">
                      <div className="flex items-center justify-center space-x-2 text-base text-gray-500">
                        <FaRegThumbsUp className="w-4 h-4 mt-1" />
                        <p>Like</p>
                      </div>
                      <div className="flex items-center justify-center space-x-2 text-base text-gray-500">
                        <LuMessageSquare className="w-4 h-4 mt-1" />
                        <p>Comment</p>
                      </div>
                      <div className="flex items-center justify-center space-x-2 text-base text-gray-500">
                        <PiShareFat className="w-4 h-4 mt-1" />
                        <p>Share</p>
                      </div>
                    </div>
                  </div>
