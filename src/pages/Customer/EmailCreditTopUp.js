...
const EmailCreditTopUp = () => {
  ...
  const [emailUnits, setEmailUnits] = useState("");
    ...
    const fetchPerUnitAmount = ()=>{
      try{
        ...
        axios.get(`${Config.apiUrl}/pricing/per-unit`, {
          ...
          setAmountPerUnit(res.data.perUnit.email);
  ...
  const handleNextStep = () => {
    if ((emailUnits === 0 || totalAmount===0) && currentStep === 1) {
      toast.error("You must select the number of email credits you are purchasing");
      return;
    }
  ...
  useEffect(() => {
    if (emailUnits) {
      setTotalAmount(parseFloat(emailUnits * amountPerUnit).toFixed(2));
    }
    if(emailUnits==="" || emailUnits===0) {
      setTotalAmount(0);
    }
  }, [emailUnits, amountPerUnit]);
          ...
          <Content>
            <ContentHeader>
              ...
              <HeaderSubTitle className="!text-center">
                Top Up your email credit by following the process.
              </HeaderSubTitle>
            </ContentHeader>
              ...
              {currentStep === 1 && (
                <div className="flex justify-end w-full max-w-3xl mt-6 ml-3">
                  <Button2
                    ...
                    type="button"
                  >
                    <IoIosArrowBack className="w-4 h-4 ml-6 " />
...
const Title = tw.h1`text-2xl  text-gray-800 whitespace-nowrap   font-medium`;
const PlanUnitsWrapper = tw.div`w-full mt-8  bg-white border   rounded-lg overflow-hidden`;
const Input2 = tw.input`  font-medium  text-gray-900 w-36`;
...
export default EmailCreditTopUp;
