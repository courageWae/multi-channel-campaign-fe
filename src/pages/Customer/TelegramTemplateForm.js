...
const TelegramTemplateForm = () => {
  ...
  const CreateTemplateFunction = async (values) => {
    await axios.post(`${Config.apiUrl}/template/telegram/create`, values, {
      headers: {
  ...
  const createTemplateSuccess = (data) => {
    navigate("/telegram-template");
    toast.success(
      data?.data?.msg || "Telegram template has been created successfully"
    );
  };
  ...
  return (
    <Container>
      <Page>
        ...
        <Contents>
          <ContentHeader>
            <HeaderTitle>Create a Telegram template</HeaderTitle>
            <HeaderSubTitle className="mt-6">
              Design a Telegram message template that can be used for your
              Telegram campaigns. A template needs to be submitted for approval
              to Telegram before it can be sent to Telegram customers.
            </HeaderSubTitle>
          </ContentHeader>
          <Formik
