...
const VoiceCampaignReport = () => {
  ...
  const fetchFunction = async () =>
    await axios.get(`${Config.apiUrl}/campaign/voice/generate/report/${id}`, {
      headers: {
  ...
  const stats = [
    ...
    { label: "Dialed", number: reportData?.dialed, color: "#d8b4fe" },
    {
      label: "Answered",
      number: reportData?.answeredCount,
      color: "#86efac",
    },
  ...
  const message = reportData?.file_name;
                ...
                <div className="md:w-1/2 w-full">
                  <div className="flex justify-between my-6">
                    ...
                    <Status>
                      ...
                      {reportData.caller_id}
                    </Status>
                  </div>
                  <Status>
                    <Label className="mb-2">Audio:</Label>
                    <Message id="message-content">
                      {isExporting && <span> {message}</span>}
                      {!isExporting && (
...
const Message = tw.div`p-4 mb-6 bg-purple-100 text-base font-normal link-break rounded-lg text-purple-900 items-center`;
...
export default VoiceCampaignReport;
